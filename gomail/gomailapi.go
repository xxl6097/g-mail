...
func SendMail(tomail, password string) (string, error) {
	var mailConf GoMail
	mailConf.Title = "OpenAiC官方"
	//这里就是我们发送的邮箱内容，但是也可以通过下面的html代码作为邮件内容
	// mailConf.Body = "坚持才是胜利，奥里给"

	//这里支持群发，只需填写多个人的邮箱即可，我这里发送人使用的是QQ邮箱，所以接收人也必须都要是
	//QQ邮箱
	mailConf.RecipientList = []string{tomail}
	mailConf.Sender = `noreplygpt@163.com`

	//这里QQ邮箱要填写授权码，网易邮箱则直接填写自己的邮箱密码，授权码获得方法在下面
	mailConf.SPassword = password //"JBSRJTTRTSIPBSMK"
	...
	m.SetHeader(`From`, m.FormatAddress(mailConf.Sender, mailConf.Title))
